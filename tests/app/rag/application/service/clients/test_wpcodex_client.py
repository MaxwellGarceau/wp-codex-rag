...
class TestWPCodexClient:
    ...
    def setup_method(self) -> None:
        ...
        ), patch("app.rag.application.service.clients.wpcodex_client.SemanticChunker"):
    ...
    def test_chunk_text_short(self) -> None:
        ...
        text = "Short text"
        chunks = self.client._chunk_text(text, chunk_size=100)
        assert len(chunks) == 1
    ...
    def test_generate_embeddings_batch_success(self) -> None:
        ...
        self.client.embedding_model.encode.return_value = mock_embeddings
    ...
    async def test_fetch_wp_docs_with_validation_error(self) -> None:
        ...
        with patch("httpx.AsyncClient") as mock_client_class:
            ...
            # Mock validation to return True, but parsing to raise validation error
            with patch.object(
                self.client, "_validate_http_response", return_value=True
            ):
                with patch.object(
                    self.client, "_parse_json_response", return_value=api_response_data
                ):
                    # Mock WordPressAPIResponse to raise validation error for first item
                    with patch(
    ...
    async def test_process_documentation_success(self) -> None:
        ...
        # Mock HTML cleaner
        self.client.html_cleaner.clean_html.side_effect = [
            "Cleaned content 1",
            "Cleaned content 2",
        ]

        # Mock semantic chunker
        self.client.semantic_chunker.chunk_text.side_effect = [
            ["Chunk 1.1", "Chunk 1.2"],
            ["Chunk 2.1"],
        ]
        ...
        with patch.object(
            ...
            with patch.object(self.client, "_fetch_wp_docs", return_value=mock_docs):
    ...
    async def test_process_documentation_empty_content(self) -> None:
        ...
        self.client.html_cleaner.clean_html.side_effect = ["Cleaned content 1", ""]

        # Mock semantic chunker
        self.client.semantic_chunker.chunk_text.return_value = ["Chunk 1.1"]
